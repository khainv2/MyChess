...
// Bật = 1 để engine chọn nước đi deterministic (luôn trả bestMoves[0]),
// phục vụ test correctness / so sánh nodes giữa các build.
// Bật = 0 để khôi phục hành vi random chọn 1 trong các nước có cùng best score.
#define ENGINE_DETERMINISTIC 1
...
Move kc::Engine::calc(const Board &chessBoard, const TimeInfo &timeInfo) {
    ...
    // Giữ nội dung TT giữa các lượt calc() — chỉ tăng age.
    // Replacement policy sẽ tự thải entry age cũ khi cần.
    tt.newSearch();
    tt.resetStats();
...
#if ENGINE_DETERMINISTIC
    // Deterministic: luôn chọn nước đầu tiên đạt bestValue
    return bestMoves[0];
#else
